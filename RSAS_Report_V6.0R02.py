...
# 主要是特征为：扫描报告解压包里面的文件夹为：vulnhostsfiles
...
while 1:
	...
	result_vulnhostHtml = r'<a href="vulnhostsfiles/(.*?)"'
	vuln_group_id = re.findall(result_vulnhostHtml,f0,re.S | re.M)
	hosts_id = vuln_group_id[index]
	f.close()
	f1 = open('./'+sys.argv[1]+'/vulnhostsfiles/'+hosts_id,'r')
	f10 = f1.read()
	hosts_all_list = f10.split(',')
	hosts_all_list_1 = map(lambda x : re.sub('^.*\n','',x),hosts_all_list)
	f1.close()
	for host in hosts_all_list_1:
